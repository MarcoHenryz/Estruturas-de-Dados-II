...
void fatorbalanceamento(PONT no) {
    if (no != NULL) {
...
PONT rotacaoRL(PONT p) {
    p->dir = rotacao_L(p->dir);
    return rotacao_R(p);
}

PONT rotacaoLR(PONT p) {
...
PONT balancear(PONT raiz) { // funcao que verifica se é necessario balancear e executa o balanceamento necessario.
    ...
    if (raiz->bf == -2) {
        ...
        if (raiz->dir->bf >= 0) {
            raiz = rotacao_R(raiz); //right right
        } else {
...
PONT inserir(PONT raiz, tipochave chave) {
    ...
    raiz = balancear(raiz); // ver se é necessario ocorrer um balanceamento
    return raiz;
}
